...
echo  "########## ANALYSIS ##########"
for src in uart_rx_fsm.vhd uart_rx_fsm-tb.vhd; do
...
echo "########## SYNTHESIS ##########"
$GHDL synth $GHDLFLAGS UART_RX_FSM >$SYNTH_FILE
if [ $? -ne 0 ]; then
...
echo "########## SIMULATION ##########"
$GHDL -c $GHDLFLAGS -r UART_RX_FSM_TB --wave=$SIM_FILE
if [ $? -ne 0 ]; then
    echo "Simulation ended with error!"
    exit 1
fi
$GTKW $SIM_FILE --script=fsm.tcl
